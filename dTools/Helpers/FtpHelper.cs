...
namespace dTools
{
    ...
    public class FTPHelper
    {
        ...
        public FTPHelper(string FtpServerIP, string FtpRemotePath, string FtpUserID, string FtpPassword)
        {
            ...
            if (!ftpServerIP.StartsWith("ftp://", StringComparison.OrdinalIgnoreCase))
            {
        ...
        /// 上传  
        /// </summary>   
        public void Upload(string localFileFullName, string remoteFileName = "")
        {
            var local = new FileInfo(localFileFullName);
            ...
            reqFTP.ContentLength = local.Length;
            if (!string.IsNullOrEmpty(remoteFileName))
            {
                reqFTP.RenameTo = remoteFileName;
            }
            int buffLength = 2048;
            byte[] buff = new byte[buffLength];
            int contentLen;
            FileStream fs = local.OpenRead();
            try
            {
        ...
        {
            url = ftpURI + url;
            List<string> retavl = new List<string>();
            FtpWebRequest reqFTP;
        ...
        {
            string[] fileList = GetFileList("*.*").ToArray();
